...
public class Expression {
    ...
    /**
     * .
     * 对函数进行简化，先把表达式通过‘+’分成几部分，然后分别对每部分进行操作；又将每部分的变量和数字分开对其进行计算后再合并输出
     *
     * @param expre (填写注释)
     * @param com   (填写注释)
     * @return
     */
    private String simplify(String expre, String com) {
        int i, j, n = 0, k;
    ...
    /**
     * .
     * 只是对以上函数进行的一点改变，先把表达式进行简化，然后再把其通过相同的方法分成几部分分别操作，然后进行降幂求导
     *
     * @param expre (填写注释)
     * @param com   (填写注释)
     */
    private void derivative(String expre, String com) {
        Expression test = new Expression();
